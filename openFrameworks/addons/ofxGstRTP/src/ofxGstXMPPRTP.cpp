...
void ofxGstXMPPRTP::setStunServer(const string & ip, guint port){
	stunServer = ip;
	stunPort = port;
}

/// add a TURN server to use for during discovery
void ofxGstXMPPRTP::addRelay(const string & ip, guint port, const string & user, const string & pwd, NiceRelayType type){
